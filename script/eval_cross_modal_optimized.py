...
def evaluate_optimized(
    ...
    cross_model = CrossModalAutoencoders(
        fd_dim=int(h["fd_dim"]) if "fd_dim" in h else (64 if model_tag == "ResNet20" else 512 if model_tag == "ResNet18" else 2048),
        ed_dim=int(h["ed_dim"]) if "ed_dim" in h else 512,
        latent_dim=int(h["latent_dim"]),
        hidden_dims=list(h["hidden_dims"]),
    ).to(device_t)
    cross_model.load_state_dict(cm_ckpt["model_state_dict"])
    cross_model.eval()

    # Build classifier and load weights
    classifier_ckpt = torch.load(classifier_path, map_location=device_t)
    classifier_hyperparams = classifier_ckpt.get("hyperparameters", {})
    dropout = classifier_hyperparams.get("dropout", 0.3)
    
    classifier = DebiasedClassifier(cross_model, num_classes=num_classes, dropout=dropout).to(device_t)
...
def main():
    p = argparse.ArgumentParser(description="Evaluate optimized cross-modal classifier using LfF debiased features.")
    ...
    p.add_argument("--cross_modal_ckpt_path", required=True, help="Path to optimized AE checkpoint with hyperparameters")
    p.add_argument("--classifier_path", required=True, help="Path to optimized trained classifier .pth")
    p.add_argument("--batch_size", type=int, default=256)
    p.add_argument("--num_workers", type=int, default=0)
    p.add_argument("--device", default="cuda")
    args = p.parse_args()

    acc = evaluate_optimized(
    ...
    print(f"Optimized Eval accuracy: {acc:.4f}")
